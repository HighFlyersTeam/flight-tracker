...
async function getAirportData() {
  return await fetch("../static/airports.json").then((response) => {
    return response.json();
  });
}

async function getAirlineColorData() {
  return await fetch("../static/airline_colors.json").then((response) => {
    return response.json();
  });
}

async function getAirlineData() {
  return await fetch("../static/airlines.json").then((response) => {
    return response.json();
  });
}
...
function clearMap() {
  for (let i = 0; i < mapMarkers.length; i++) mapMarkers[i].setMap(null);
  for (let i = 0; i < mapPaths.length; i++) mapPaths[i].setMap(null);
  mapMarkers = [];
  mapPaths = [];
}
...
export function updateMapWithFlights(flightData) {
  ...
  for (let i = 0; i < flightData.length; i++) {
    ...
    const destinationAirport = flightData[i][1];
    currentPath.push({
      lat: airportData[originAirport]["lat"],
      lng: airportData[originAirport]["lng"],
    });
    currentPath.push({
      lat: airportData[destinationAirport]["lat"],
      lng: airportData[destinationAirport]["lng"],
    });
    ...
    let information =
      ...
      airlineData[airline] +
      "<br />";
    if (flightData[i][3] === "true") information += "Cargo airplane";
    else if (flightData[i][3] === "false") information += "Passenger airplane";
