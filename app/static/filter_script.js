...
document.addEventListener("DOMContentLoaded", () => {
  ...
  advancedFilterStartDatetime = document.getElementById(
...
async function setFilterMenuData() {
  const airlineData = await fetch("../static/airlines.json").then(
    (response) => {
  ...
  const airportData = await fetch("../static/airports.json").then(
    (response) => {
      return response.json();
    }
  );
